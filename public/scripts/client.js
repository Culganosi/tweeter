...
const createTweetElement = function(tweetObject) {
  ...
  <article class="tweet">
    <header>
      ...
      <h2 class="handle">${escape(tweetObject.user.name)}</h2>
      <p class="name">${escape(tweetObject.user.handle)}</p>
    </header>
    <body>
...
let addTweet = () => {
    $('form').on('submit', function(event) {
        ...
        if ($("#tweet-text").val().length > 140) {
            ...
            $('.error-message').text("Error! Please input text into the field").slideDown();
        } else {
